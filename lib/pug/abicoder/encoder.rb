module Pug
  module Abicoder
    ##
    # Contract Abicoder encoding and decoding.
    #
    ...
    class Encoder
      ...
      def encode(types, args)
        ...
        head = ''.b
        tail = ''.b ## note: use string buffer with BINARY / ASCII_8BIT encoding!!!
      ...
      #
      # @param type [Abicoder::Type] value type
      ...
      def encode_dynamic_array(type, args)
        ...
        if type.is_a?(Array) ## dynamic array
          ...
          unless args.size == type.dim
            raise ArgumentError,
                  "Wrong array size: found #{args.size}, expecting #{type.dim}"
          end
        end
      ...
      def encode_tuple(tuple, args)
        ...
        unless args.size == tuple.types.size
          raise ArgumentError,
                "Wrong array size (for tuple): found #{args.size}, expecting #{tuple.type.size} tuple elements"
        end
      ...
      def encode_uint256(arg) = encode_uint(arg, 256)
